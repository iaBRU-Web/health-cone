module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── Two AI backends available, tried in order ──
    // 1. multi-ai-backend.vercel.app  — the real AI backend (holds Groq API keys)
    // 2. health-cone-backend.vercel.app — your own HealthCone backend as fallback
    const AI_BACKENDS = [
      'https://multi-ai-backend.vercel.app/api/chat',
      'https://health-cone-backend.vercel.app/api/chat',
    ];
    ...
    // Try every model slot on primary backend first, then fallback backend
    for (const backendUrl of AI_BACKENDS) {
      for (const m of modelsToTry) {
        ...
        try {
          ...
          // Network error — try next model/backend
          continue;
        }
        ...
        // Extract the AI reply text from whatever shape the backend returns
        let replyText = '';
    ...
    // All backends and models failed
